...
class TestNormalizer extends SerializerAwareNormalizer implements DenormalizerInterface, NormalizerInterface
{
    ...
    public function denormalize($data, $class, $format = null, array $context = [])
    {
        ...
        if (property_exists($data, 'date')) {
            $object->setDate(\DateTime::createFromFormat('Y-m-d', $data->{'date'}));
        }
        if (property_exists($data, 'dateOrNull')) {
            $value = $data->{'dateOrNull'};
            if (is_string($data->{'dateOrNull'}) and false !== \DateTime::createFromFormat('Y-m-d', $data->{'dateOrNull'})) {
                $value = \DateTime::createFromFormat('Y-m-d', $data->{'dateOrNull'});
            }
        ...
        if (property_exists($data, 'dateOrNullOrInt')) {
            ...
            if (is_string($data->{'dateOrNullOrInt'}) and false !== \DateTime::createFromFormat('Y-m-d', $data->{'dateOrNullOrInt'})) {
                $value_1 = \DateTime::createFromFormat('Y-m-d', $data->{'dateOrNullOrInt'});
            }
